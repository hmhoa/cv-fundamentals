...
def RGBtoHSV(image):
    ...
    for i in range(row):
        for j in range(col):
            ...
            #calculate value
            v = np.max(image[i, j])/255

            #compute chroma
            c = v - np.min(image[i, j])/255
...
def main(args):
    ...
    file_name = args[1]
    img = io.imread(file_name) #skimage img already represented as a numpy array
    ...
    #do hsv modifications - assumed that we are doing increase changes
